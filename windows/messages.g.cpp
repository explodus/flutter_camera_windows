...
void CameraEventApi::FrameAvailabledEvent(
  int64_t camera_id_arg,
  const std::vector<uint8_t>& image_bytes_arg,
  std::function<void(void)>&& on_success,
  std::function<void(const FlutterError&)>&& on_error) {
  const std::string channel_name = "dev.flutter.pigeon.camera_windows.CameraEventApi.frameAvailabledEvent" + message_channel_suffix_;
  BasicMessageChannel<> channel(binary_messenger_, channel_name, &GetCodec());
  EncodableValue encoded_api_arguments = EncodableValue(EncodableList{
    EncodableValue(camera_id_arg),
    EncodableValue(image_bytes_arg),
  });
